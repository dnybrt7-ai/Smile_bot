...
async function callRoom(env: Env, chatId: number, path: string, body: unknown) {
  console.log(`[CALL_ROOM] START path=${path} chatId=${chatId}`);

  try {
    const stub = getRoomStub(env, chatId);

    const response = await stub.fetch(`https://game-room${path}`, {
      method: 'POST',
    ...
    console.log(
      `[CALL_ROOM] RESPONSE path=${path} chatId=${chatId} status=${response.status}`
    );

    return response;
  } catch (error) {
    console.error(
      `[CALL_ROOM] ERROR path=${path} chatId=${chatId}`,
      error
    );
    throw error;
  }
